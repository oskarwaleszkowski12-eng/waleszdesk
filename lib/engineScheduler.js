...
function isSafeUrl(rawUrl) {
  try {
    ...
    const h = u.hostname;
    if (/^(localhost|127\.|10\.|192\.168\.|172\.(1[6-9]|2\d|3[01])\.|169\.254\.|::1$|fc00:|fd)/.test(h)) return false;
    return true;
  } catch { return false; }
}
          ...
          if (item.external_id) {
            await pool.query(
              `INSERT INTO engine_research_items (source_id, author, content, url, external_id, published_at)
               VALUES ($1,$2,$3,$4,$5,$6) ON CONFLICT (source_id, external_id) WHERE external_id IS NOT NULL DO NOTHING`,

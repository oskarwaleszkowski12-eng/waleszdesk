...
// In-memory fallback (LRU-ish: prune on miss + bounded eviction on insert)
...
function memEvictIfFull() {
  ...
  // Map iteration is insertion-ordered → drop ~10% oldest entries
  const toDrop = Math.ceil(MEM_CACHE_MAX * 0.1);
  let i = 0;

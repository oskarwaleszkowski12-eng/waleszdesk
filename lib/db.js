...
async function initDb() {
  ...
  `);
  await pool.query(`CREATE INDEX IF NOT EXISTS webhook_events_bot_created ON webhook_events(bot_id, created_at DESC)`);
  await pool.query(`CREATE INDEX IF NOT EXISTS webhook_events_status_idx  ON webhook_events(status, created_at DESC)`);
  logger.info('[DB] webhook_events ready');
